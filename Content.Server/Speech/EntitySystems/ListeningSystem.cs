...
public sealed class ListeningSystem : EntitySystem
{
    ...
    {
        PingListeners(ev.Source, ev.Message, ev.ObfuscatedMessage, ev.Language);    // Horizon languages
    }

    public void PingListeners(EntityUid source, string message, string? obfuscatedMessage, LanguagePrototype language)  // Horizon languages
    {
        ...
        var ev = new ListenEvent(message, source, language);    // Horizon languages
        var obfuscatedEv = obfuscatedMessage == null ? null : new ListenEvent(obfuscatedMessage, source, language); // Horizon languages
