...
public partial interface ILanguageType
{
    ...
    void Speak(EntityUid uid, string message, string name, SpeechVerbPrototype verb, byte range, IEntityManager entMan, out bool success, out string resultMessage);

    void Whisper(EntityUid uid, string message, string name, string nameIdentity, byte range, IEntityManager entMan, out bool success, out string resultMessage, out string resultObfMessage);
}
